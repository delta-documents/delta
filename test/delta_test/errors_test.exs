defmodule DeltaTest.ErrorsTest do
  ...
  test "Delta.Errors.DoesNotExist" do
    assert "A with id = 0 does not exist. msg" == to_string(%DoesNotExist{struct: A, id: 0, message: "msg"})
  end

  test "Delta.Errors.AlreadyExist" do
    assert "A with id = 0 already exists. msg" == to_string(%AlreadyExist{struct: A, id: 0, message: "msg"})
  end

  test "Delta.Errors.Validation" do
    assert "Expected A.a to be one, got: another. msg" == to_string(%Validation{struct: A, field: :a, expected: "one", got: "another", message: "msg"})
  end
